...
let _cachedRamaTitlesIndex = null;
let _ramaCachePages = {};

function _loadRamaTitlesIndex() {
  if (_cachedRamaTitlesIndex) return _cachedRamaTitlesIndex;
  try {
  ...
  return _cachedRamaTitlesIndex;
}

function _loadRamaCachePage(pageNum) {
  if (_ramaCachePages[pageNum]) return _ramaCachePages[pageNum];
  try {
    const pathMod = require('path');
    const fs = require('fs');
    const zlib = require('zlib');
    const gzPath = pathMod.resolve(__dirname, '..', '..', 'rama-cache', `page-${pageNum}.json.gz`);
    if (fs.existsSync(gzPath)) {
      const data = JSON.parse(zlib.gunzipSync(fs.readFileSync(gzPath)));
      _ramaCachePages[pageNum] = data;
      return data;
    }
    const jsonPath = pathMod.resolve(__dirname, '..', '..', 'rama-cache', `page-${pageNum}.json`);
...
async function getCatalog(skip = 0, search = '', config = {}) {
  ...
  // Try pre-built .gz cache first (generated by warm-rama-cache.js)
  let usedGzCache = false;
  let pageNumber = startPage;
  while (items.length < ITEMS_PER_PAGE && pageNumber <= MAX_PAGES) {
    const cachedPage = _loadRamaCachePage(pageNumber);
    if (!cachedPage) break; // no more .gz pages, fall through to live
    for (const card of cachedPage) {
      const slug = card.slug || '';
      const title = card.title || '';
      const id = `rama_${slug}`;
      if (searchQuery && !title.toLowerCase().includes(searchQuery) && !slug.includes(searchQuery)) continue;
      items.push({ id, type: 'series', name: title, poster: card.poster || '', posterShape: 'poster' });
    }
    usedGzCache = true;
    if (cachedPage.length < 5) break;
    pageNumber++;
  }

  if (usedGzCache && items.length > 0) {
    ...
    log.info(`catalog from .gz cache: ${result.length} items`, { skip, search });
    return result;
  }

  // Fallback: live HTML scraping
  items.length = 0;
