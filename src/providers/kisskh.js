...
function _loadKkTitlesIndex() {
  if (_cachedKkTitlesIndex) return _cachedKkTitlesIndex;
  try {
...
function _loadKkCachePage(pageNum) {
  if (_kkCachePages[pageNum]) return _kkCachePages[pageNum];
  try {
    const path = require('path');
    const fs = require('fs');
    const zlib = require('zlib');
    const gzPath = path.resolve(__dirname, '..', '..', 'kk-cache', `page-${pageNum}.json.gz`);
    if (fs.existsSync(gzPath)) {
      const data = JSON.parse(zlib.gunzipSync(fs.readFileSync(gzPath)));
      _kkCachePages[pageNum] = data;
      return data;
    }
    const jsonPath = path.resolve(__dirname, '..', '..', 'kk-cache', `page-${pageNum}.json`);
...
async function _listCatalog(page, limit, proxyUrl, clientIp) {
  // Try pre-built .gz cache first (generated by warm-kk-cache.js)
  const cachedPage = _loadKkCachePage(page);
  if (cachedPage && cachedPage.length > 0) {
    log.info(`list catalog from .gz cache page ${page}: ${cachedPage.length} items`);
    // Raw API items need mapping through _mapItem
    const mapped = cachedPage.map(_mapItem);
...
async function _searchCatalog(query, limit = 20, proxyUrl, clientIp) {
  // Try pre-built titles index first (generated by warm-kk-cache.js)
  const kkIndex = _loadKkTitlesIndex();
  if (kkIndex) {
    const cleanQuery = cleanTitleForSearch(query).toLowerCase();
    const matches = [];
    for (const [key, entries] of Object.entries(kkIndex)) {
      if (key.includes(cleanQuery) || cleanQuery.includes(key)) {
        for (const entry of entries) {
          // Load the page from .gz to get full item data
          const pageData = _loadKkCachePage(entry.page);
          if (pageData) {
            const rawItem = pageData.find(i => i.id === entry.id);
            if (rawItem) matches.push(_mapItem(rawItem));
          }
        }
      }
    }
    if (matches.length > 0) {
      log.info(`search from .gz index: ${matches.length} matches for "${query}"`);
      const seen = new Set();
      return matches.filter(i => { if (seen.has(i.id)) return false; seen.add(i.id); return true; }).slice(0, limit);
    }
  }
